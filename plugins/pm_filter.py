...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, REQ_CHANNEL_1, REQ_CHANNEL_2, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, URL_SHORTENR_WEBSITE, URL_SHORTNER_WEBSITE_API
...
async def advantage_spoll_choker(bot, query):
    ...
    if query.message.reply_to_message:
        message_id = query.message.reply_to_message.id
    else:
        # Handle bot PM where reply_to_message is not available
        message_id = query.message.id
    movies = SPELL_CHECK.get(message_id)
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit(f"\n👋 Hello {reqstr.mention},\n\n <b> Your Movie: </b><b>'{movie}'</b><b> \n \n Will Be Uploaded Here 👇 Join Now </b>",
                                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⭕ Join Here ⭕", url="https://t.me/+ejmG5IL_CiI3MWM1")]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Join Both Channels & Click Get File ! 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            InlineKeyboardButton('▫️ ᴍᴀɴᴜᴀʟ FIʟᴛᴇʀ ▫️', callback_data='manuelfilter'),
            InlineKeyboardButton('▪️ Aᴜᴛᴏ FIʟᴛᴇʀ ▪️', callback_data='autofilter')
        ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('♻️ Fɪʟᴇ Sᴛᴏʀᴇ ♻️', callback_data='store_file')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton(' 🔻 Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ 🔻', url=GRP_LNK),
            InlineKeyboardButton('▪️ Sᴏᴜʀᴄᴇ Cᴏᴅᴇ ▪️', callback_data='source')
        ],[
            InlineKeyboardButton('🔰 Hᴏᴍᴇ 🔰', callback_data='start'),
        ...
        buttons = [[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('♻️ Rᴇғʀᴇsʜ ♻️', callback_data='rfrsh')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
